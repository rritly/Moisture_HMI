...
macroDelay = TON0(macro, 35)

//фронт сигнала пуска макроса
macroRaise = rise32(macroDelay)

//Берем переменные из контроллера
GetData(status, "Local HMI", LW, 80, 1)
GetData(constDistance, "Local HMI", LW, 82, 1)
GetData(currDistance, "Local HMI", LW, 84, 1)
GetData(currMoisture, "Local HMI", LW, 96, 1)
...
if pauseRaise or pauseFall then
    SetData(pause, "Local HMI", LB, 235, 1)
else
end if
...
if enableFall3  then 
    ...
    moisture23 = false
    Notmoisture0 = false
    SetData(Notmoisture0, "Local HMI", LB, 210, 1)
    Notmoisture1 = false
    SetData(Notmoisture0, "Local HMI", LB, 211, 1)
    Notmoisture2 = false
    SetData(Notmoisture0, "Local HMI", LB, 212, 1)
    Notmoisture3 = false
    SetData(Notmoisture0, "Local HMI", LB, 213, 1)
    Notmoisture4 = false
    SetData(Notmoisture0, "Local HMI", LB, 214, 1)
    Notmoisture5 = false
    SetData(Notmoisture0, "Local HMI", LB, 215, 1)
    Notmoisture6 = false
    SetData(Notmoisture0, "Local HMI", LB, 216, 1)
    Notmoisture7 = false
    SetData(Notmoisture0, "Local HMI", LB, 217, 1)
    Notmoisture8 = false
    SetData(Notmoisture0, "Local HMI", LB, 218, 1)
    Notmoisture9 = false
    SetData(Notmoisture0, "Local HMI", LB, 219, 1)
    Notmoisture10 = false
    SetData(Notmoisture0, "Local HMI", LB, 220, 1)
    Notmoisture11 = false
    SetData(Notmoisture0, "Local HMI", LB, 221, 1)
    Notmoisture12 = false
    SetData(Notmoisture0, "Local HMI", LB, 222, 1)
    Notmoisture13 = false
    SetData(Notmoisture0, "Local HMI", LB, 223, 1)
    Notmoisture14 = false
    SetData(Notmoisture0, "Local HMI", LB, 224, 1)
    Notmoisture15 = false
    SetData(Notmoisture0, "Local HMI", LB, 225, 1)
    Notmoisture16 = false
    SetData(Notmoisture0, "Local HMI", LB, 226, 1)
    Notmoisture17 = false
    SetData(Notmoisture0, "Local HMI", LB, 227, 1)
    Notmoisture18 = false
    SetData(Notmoisture0, "Local HMI", LB, 228, 1)
    Notmoisture19 = false
    SetData(Notmoisture0, "Local HMI", LB, 229, 1)
    Notmoisture20 = false
    SetData(Notmoisture0, "Local HMI", LB, 230, 1)
    Notmoisture21 = false
    SetData(Notmoisture0, "Local HMI", LB, 231, 1)
    Notmoisture22 = false
    SetData(Notmoisture0, "Local HMI", LB, 232, 1)
    Notmoisture23 = false
    SetData(Notmoisture0, "Local HMI", LB, 233, 1)
else
end if
...
if macro and macroDelay and not pause then
    ...
    if graf0Raise or enableFall2 then
        ...
        if not allMoisture then 
            SetData(graf0, "Local HMI", LB, 171, 1)
        else
        end if
    else
    end if
    if graf1Raise or enableFall2 then
        if errMoisture or NotMoisture1 then
            SetData(errMoisture, "Local HMI", LB, 211, 1)
            NotMoisture1 = errMoisture
        else
        end if	
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 102, 1)
            moisture1 = true
        else
        end if
        if not allMoisture then 
            SetData(graf1, "Local HMI", LB, 172, 1)
        else
        end if
    else
    end if
    if graf2Raise or enableFall2 then
        if errMoisture or NotMoisture2 then
            SetData(errMoisture, "Local HMI", LB, 212, 1)
            NotMoisture2 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 104, 1)
            moisture2 = true
        else
        end if
        if not allMoisture then 
            SetData(graf2, "Local HMI", LB, 173, 1)
        else
        end if
    else
    end if
    if graf3Raise or enableFall2 then
        if errMoisture or NotMoisture3 then
            SetData(errMoisture, "Local HMI", LB, 213, 1)
            NotMoisture3 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 106, 1)
            moisture3 = true
        else
        end if
                if not allMoisture then 
            SetData(graf3, "Local HMI", LB, 174, 1)
        else
        end if
    else
    end if
    if graf4Raise or enableFall2 then
        if errMoisture or NotMoisture4 then
            SetData(errMoisture, "Local HMI", LB, 214, 1)
            NotMoisture4 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 108, 1)
            moisture4 = true
        else
        end if
        if not allMoisture then 
            SetData(graf4, "Local HMI", LB, 175, 1)
        else
        end if
    else
    end if
    if graf5Raise or enableFall2 then
        if errMoisture or NotMoisture5 then
            SetData(errMoisture, "Local HMI", LB, 215, 1)
            NotMoisture5 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 110, 1)
            moisture5 = true
        else
        end if
        if not allMoisture then 
            SetData(graf5, "Local HMI", LB, 176, 1)
        else
        end if
    else
    end if
    if graf6Raise or enableFall2 then
        if errMoisture or NotMoisture6 then
            SetData(errMoisture, "Local HMI", LB, 216, 1)
            NotMoisture6 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 112, 1)
            moisture6 = true
        else
        end if
        if not allMoisture then 
            SetData(graf6, "Local HMI", LB, 177, 1)
        else
        end if
    else
    end if
    if graf7Raise or enableFall2 then
        if errMoisture or NotMoisture7 then
            SetData(errMoisture, "Local HMI", LB, 217, 1)
            NotMoisture7 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 114, 1)
            moisture7 = true
        else
        end if
        if not allMoisture then 
            SetData(graf7, "Local HMI", LB, 178, 1)
        else
        end if
    else
    end if
    if graf8Raise or enableFall2 then
        if errMoisture or NotMoisture8 then
            SetData(errMoisture, "Local HMI", LB, 218, 1)
            NotMoisture8 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 116, 1)
            moisture8 = true
        else
        end if
        if not allMoisture then 
            SetData(graf8, "Local HMI", LB, 179, 1)
        else
        end if
    else
    end if
    if graf9Raise or enableFall2 then
        if errMoisture or NotMoisture9 then
            SetData(errMoisture, "Local HMI", LB, 219, 1)
            NotMoisture9 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 118, 1)
            moisture9 = true
        else
        end if
        if not allMoisture then 
            SetData(graf9, "Local HMI", LB, 180, 1)
        else
        end if
    else
    end if
    if graf10Raise or enableFall2 then
        if errMoisture or NotMoisture10 then
            SetData(errMoisture, "Local HMI", LB, 220, 1)
            NotMoisture10 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 120, 1)
            moisture10 = true
        else
        end if
        if not allMoisture then 
            SetData(graf10, "Local HMI", LB, 181, 1)
        else
        end if
    else
    end if
    if graf11Raise or enableFall2 then
        if errMoisture or NotMoisture11 then
            SetData(errMoisture, "Local HMI", LB, 221, 1)
            NotMoisture11 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 122, 1)
            moisture11 = true
        else
        end if
        if not allMoisture then 
            SetData(graf11, "Local HMI", LB, 182, 1)
        else
        end if
    else
    end if
    if graf12Raise or enableFall2 then
        if errMoisture or NotMoisture12 then
            SetData(errMoisture, "Local HMI", LB, 222, 1)
            NotMoisture12 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 124, 1)
            moisture12 = true
        else
        end if
        if not allMoisture then 
            SetData(graf12, "Local HMI", LB, 183, 1)
        else
        end if
    else
    end if
    if graf13Raise or enableFall2 then
        if errMoisture or NotMoisture13 then
            SetData(errMoisture, "Local HMI", LB, 223, 1)
            NotMoisture13 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 126, 1)
            moisture13 = true
        else
        end if
        if not allMoisture then 
            SetData(graf13, "Local HMI", LB, 184, 1)
        else
        end if
    else
    end if
    if graf14Raise or enableFall2 then
        if errMoisture or NotMoisture14 then
            SetData(errMoisture, "Local HMI", LB, 224, 1)
            NotMoisture14 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 128, 1)
            moisture14 = true
        else
        end if
        if not allMoisture then 
            SetData(graf14, "Local HMI", LB, 185, 1)
        else
        end if
    else
    end if
    if graf15Raise or enableFall2 then
        if errMoisture or NotMoisture15 then
            SetData(errMoisture, "Local HMI", LB, 225, 1)
            NotMoisture15 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 130, 1)
            moisture15 = true
        else
        end if
        if not allMoisture then 
            SetData(graf15, "Local HMI", LB, 186, 1)
        else
        end if
    else
    end if
    if graf16Raise or enableFall2 then
        if errMoisture or NotMoisture16 then
            SetData(errMoisture, "Local HMI", LB, 226, 1)
            NotMoisture16 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 132, 1)
            moisture16 = true
        else
        end if
        if not allMoisture then 
            SetData(graf16, "Local HMI", LB, 187, 1)
        else
        end if
    else
    end if
    if graf17Raise or enableFall2 then
        if errMoisture or NotMoisture17 then
            SetData(errMoisture, "Local HMI", LB, 227, 1)
            NotMoisture17 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 134, 1)
            moisture17 = true
        else
        end if
        if not allMoisture then 
            SetData(graf17, "Local HMI", LB, 188, 1)
        else
        end if
    else
    end if
    if graf18Raise or enableFall2 then
        if errMoisture or NotMoisture18 then
            SetData(errMoisture, "Local HMI", LB, 228, 1)
            NotMoisture18 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 136, 1)
            moisture18 = true
        else
        end if
        if not allMoisture then 
            SetData(graf18, "Local HMI", LB, 189, 1)
        else
        end if
    else
    end if
    if graf19Raise or enableFall2 then
        if errMoisture or NotMoisture19 then
            SetData(errMoisture, "Local HMI", LB, 229, 1)
            NotMoisture19 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 138, 1)
            moisture19 = true
        else
        end if
        if not allMoisture then 
            SetData(graf19, "Local HMI", LB, 190, 1)
        else
        end if
    else
    end if
    if graf20Raise or enableFall2 then
        if errMoisture or NotMoisture20 then
            SetData(errMoisture, "Local HMI", LB, 230, 1)
            NotMoisture20 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 140, 1)
            moisture20 = true
        else
        end if
        if not allMoisture then 
            SetData(graf20, "Local HMI", LB, 191, 1)
        else
        end if
    else
    end if
    if graf21Raise or enableFall2 then
        if errMoisture or NotMoisture21 then
            SetData(errMoisture, "Local HMI", LB, 231, 1)
            NotMoisture21 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 142, 1)
            moisture21 = true
        else
        end if
        if not allMoisture then 
            SetData(graf21, "Local HMI", LB, 192, 1)
        else
        end if
    else
    end if
    if graf22Raise or enableFall2 then
        if errMoisture or NotMoisture22 then
            SetData(errMoisture, "Local HMI", LB, 232, 1)
            NotMoisture22 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 144, 1)
            moisture22 = true
        else
        end if
        if not allMoisture then 
            SetData(graf22, "Local HMI", LB, 193, 1)
        else
        end if
    else
    end if
    if graf23Raise or enableFall2 then
        if errMoisture or NotMoisture23 then
            SetData(errMoisture, "Local HMI", LB, 233, 1)
            NotMoisture23 = errMoisture
        else
        end if
        if not errMoisture and enable then
            SetData(currMoisture, "Local HMI", LW, 146, 1)
            moisture23 = true
        else
        end if
        if not allMoisture then 
            SetData(graf23, "Local HMI", LB, 194, 1)
        else
        end if
    else
    end if
...
if enableRaise or macroRaise then
    SetData(pointDistance55, "Local HMI", LW, 180, 1)
    SetData(pointDistance44, "Local HMI", LW, 182, 1)
    SetData(pointDistance33, "Local HMI", LW, 184, 1)
    SetData(pointDistance22, "Local HMI", LW, 186, 1)
    SetData(pointDistance11, "Local HMI", LW, 188, 1)
    SetData(pointDistance00, "Local HMI", LW, 190, 1)
    SetData(constDistance, "Local HMI", LW, 192, 1)
else
end if
